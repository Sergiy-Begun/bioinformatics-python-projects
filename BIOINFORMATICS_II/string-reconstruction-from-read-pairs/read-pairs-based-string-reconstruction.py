...
def graph_vertices_based_on_prefixes_for_read_pair(list_of_k_mers):
    """
    returns the dictionary of graph of the k-mers for read-pair case
    ...
    kmer_length = int((len(list_of_k_mers[0]) - 1) / 2)
    #print("kmer_length = ", kmer_length)

    # formation of tuples (edge,suffix)
    
    for k_mer in list_of_k_mers:
        k_mer_prefix = str(k_mer[0:(kmer_length - 1)]) + "|" + str(k_mer[(kmer_length + 1):(len(k_mer) - 1)])
        k_mer_suffix = str((k_mer[1:kmer_length])) + "|" + str(k_mer[(kmer_length + 2):len(k_mer)])
        if (k_mer_prefix in vertices_dictionary.keys()):
            vertices_dictionary[k_mer_prefix].append(k_mer_suffix)
            
        if (k_mer_prefix not in vertices_dictionary.keys()):
            vertices_dictionary[k_mer_prefix] = [k_mer_suffix]
...
def de_broijn_graph_balancer(potentially_unbalanced_graph):
    ...
    if (len(inlet_disbalanced.values()) > 0):
    ...
    if (len(inlet_disbalanced.values()) != 0):
        ...
        random_vertice = random.randint(0, (len(potentially_unbalanced_graph.keys()) - 1 ))
        artificial_vertice = list(potentially_unbalanced_graph.keys())[random_vertice]
        artificial_outlet = list(potentially_unbalanced_graph.keys())[random_vertice]
    ...
    edge_for_deletion_in_future_eulerian_cycle_to_form_eulerian_path = str(artificial_vertice) + str(artificial_outlet)
...
def eulerian_path_formation_from_eulerian_cycle(eulerian_cycle_input, artificially_injected_edge):
    ...
    for i in range(0,(length_of_eulerian_cycle - 1)):
        ...
        if (searching_for_artificially_injected_edge == artificially_injected_edge):
            eulerian_path_results.append([])
            eulerian_path_results[tr].extend(eulerian_cycle_input[(i + 1):(length_of_eulerian_cycle + 1)])
            eulerian_path_results[tr].extend(eulerian_cycle_input[1:(i + 1)])
            tr += 1
    #print("eulerian_path_results = ", eulerian_path_results)
    #print("length of eulerian_path_results = ", len(eulerian_path_results))
    
    return eulerian_path_results[0]

def genom_string_from_eulerian_path(eulerian_path_input,k_mer_length_input: int,d_size_distance: int):
    length_of_eulerian_path_input = len(eulerian_path_input)
    eulerian_path_output_string = ""
    for i in range(0,(length_of_eulerian_path_input)):
        eulerian_path_output_string += str(eulerian_path_input[i][0])
    
    #print("eulerian_path_output_string first approximation = ", eulerian_path_output_string)
    
    shift_reading = d_size_distance + k_mer_length_input
    #print("shift_reading before = ", shift_reading)
    
    delta_shift = length_of_eulerian_path_input - shift_reading
    
    #print("delta_shift = ", delta_shift)
    
    if (delta_shift < 0):
        if (abs(delta_shift) > (k_mer_length_input - 1)):
            raise ValueError("Something exremely wrong. Algorithm should be improved.")
        eulerian_path_output_string += str(eulerian_path_input[(length_of_eulerian_path_input - 1)][1:(1 + abs(delta_shift))])
        shift_reading = length_of_eulerian_path_input
    #print("shift_reading = ", shift_reading)
    #print("eulerian_path_output_string second approximation = ", eulerian_path_output_string)
    for i in range((length_of_eulerian_path_input - (shift_reading)),(length_of_eulerian_path_input - 1)):
        #print("i = ", i, "str(eulerian_path_input[",i,"][k_mer_length_input]) = ", str(eulerian_path_input[i][k_mer_length_input]))
        eulerian_path_output_string += str(eulerian_path_input[i][k_mer_length_input])
    eulerian_path_output_string += str(eulerian_path_input[length_of_eulerian_path_input - 1][k_mer_length_input:(k_mer_length_input + k_mer_length_input)])
    #print("eulerian_path_output_string final approximation = ", eulerian_path_output_string)
    return eulerian_path_output_string
...
string_with_numbers = str(read_strings_from_file[0]).split()

k_mer_length_from_file = int(string_with_numbers[0].strip())

d_size_from_file = int(string_with_numbers[1].strip())

k_mers_list = str(read_strings_from_file[1]).split()

for m in range(0,len(k_mers_list)):
    k_mers_list[m] = str(k_mers_list[m]).strip()

vertices_dictionary_output = graph_vertices_based_on_prefixes_for_read_pair(k_mers_list)

#print("vertices_dictionary_output = ", vertices_dictionary_output)

balaced_graph_result = de_broijn_graph_balancer(vertices_dictionary_output)
...
eulerian_path_output_string = genom_string_from_eulerian_path(eulerian_path_output,k_mer_length_from_file,d_size_from_file)
    
#print("eulerian_path_output_string = ", eulerian_path_output_string)

output_file = open("output_eulerian_path_based_for_read_pair_case_string.txt", "w")

output_file.write(str(eulerian_path_output_string).replace("\"", "").replace("\'","").replace("\"", "").replace("\'","").replace(",","").replace("[", "").replace("]", ""))
