...
def eulerian_path_finder(vertices_dictionary_input):
    full_list_of_edges = []
    full_starting_positions = []
    starting_positions_with_edges = {}
    
    for cur_key in vertices_dictionary_input.keys():
        full_starting_positions.append(str(cur_key))
        ...
        for i in range(0,(len(current_de_bruijn_graph_string))):
            ...
            full_starting_positions.append(current_de_bruijn_graph_string[i])
...
read_data_from_file = open("dataset_30187_2.txt", "r")    
...
eularian_path = eulerian_path_finder(vertices_dictionary)

output_file = open("output_simple_eulerian_path.txt", "w")

output_file.write(str(eularian_path).replace("\"", "").replace("\'","").replace("\"", "").replace("\'","").replace(",","").replace("[", "").replace("]", ""))
