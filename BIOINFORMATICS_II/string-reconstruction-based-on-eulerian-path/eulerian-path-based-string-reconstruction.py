...
def generate_binary_k_mers(k_mer_length):
    first_binary_element = ""
    for i in range(0,(k_mer_length)):
        first_binary_element += "0"
    binary_k_mers_list = [first_binary_element]
    
    j = 0
    while j < k_mer_length:
        prev = copy.deepcopy(binary_k_mers_list)
        for t in range(0,len(prev)):
            new_binary_element = str(prev[t][0:j]) + "1" + str(prev[t][(j + 1):(k_mer_length + 1)])
            binary_k_mers_list.append(new_binary_element)
        j += 1
    
    #print("binary_k_mers_list", binary_k_mers_list)
    
    return binary_k_mers_list
...
def de_broijn_graph_balancer(potentially_unbalanced_graph):
    ...
    if (len(inlet_disbalanced.values()) == 0):
        return ({"direct_assembly": "should_be_applied"}, "None")
...
def genom_string_from_eulerian_path(eulerian_path_input):
    length_of_eulerian_path_input = len(eulerian_path_input)
    first_element_of_string = eulerian_path_input[0]
    eulerian_path_output_string = first_element_of_string
    for i in range(1,(length_of_eulerian_path_input)):
        eulerian_path_output_string += str(eulerian_path_input[i][-1])
    return eulerian_path_output_string

def universal_string_from_eulerian_cycle(eulerian_cycle_input):
    length_of_eulerian_cycle_input = len(eulerian_cycle_input)
    eulerian_cycle_output_string = ""
    for i in range(1,(length_of_eulerian_cycle_input)):
        eulerian_cycle_output_string += str(eulerian_cycle_input[i][-1])
    return eulerian_cycle_output_string





find_universal_cycle = False

if find_universal_cycle == False:
    ...
    k_mers_list = generate_binary_k_mers(9)
...
if find_universal_cycle == True:
    eulerian_cycle = eulerian_cycle_finder(vertices_dictionary_output)
    
    #print("eulerian_cycle = ", eulerian_cycle)
        
    eulerian_path_output_string = universal_string_from_eulerian_cycle(eulerian_cycle)
    
else:
    if ("direct_assembly" not in balanced_graph.keys()):
        ...
        eulerian_path_output_string = genom_string_from_eulerian_path(eulerian_path_output)
    else:
        
        eulerian_path_output_string = simple_genom_sewing(k_mers_list)
...
output_file = open("output_eulerian_path_based_string.txt", "w")
