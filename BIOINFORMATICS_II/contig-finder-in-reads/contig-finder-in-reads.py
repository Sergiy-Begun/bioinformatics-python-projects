...
def graph_vertices_based_on_prefixes_for_read_pair(list_of_k_mers):
    ...
    kmer_length = len(list_of_k_mers[0])
    #print("kmer_length = ", kmer_length)

    # formation of tuples (edge,suffix)
    
    for k_mer in list_of_k_mers:
        k_mer_prefix = str(k_mer[0:(kmer_length - 1)])
        k_mer_suffix = str((k_mer[1:kmer_length]))
...
def contig_finder_function(de_bruijn_graph):
    ...
    for i in range(0,len(list_of_de_gruijn_graph_vertices)):
        ...
        for current_next_vertice in disbalance_viewer_dictionary[current_key_vertice][1]:
            ...
            current_contig = str(current_key_vertice)[0]
            ...
            while next_vertice_available == True:
                ...
                if (current_next_vertice_inlet_counts == 1) and (current_next_vertice_outlet_counts == 1):
                    ...
                    #print("current_contig = ", current_contig)
                    #print("next_vertice = ", next_vertice)
                    
                    current_contig += str(next_vertice)[0]
    ...
    if (len(isolated_vertices_account) > 0):
        ...
        while (len(isolated_vertices_account) > 0):
            ...
            if (next_vertice == current_next_vertice):
                current_contig = str(current_next_vertice)[0] + str(next_vertice)
            else:
                current_contig = str(current_next_vertice)[0] + str(next_vertice)[0]
...
read_data_from_file = open("dataset_30189_5.txt", "r")
...
contig_list_result = contig_finder_function(vertices_dictionary_output)

print("contig_list_result = ", contig_list_result)

print("len(contig_list_result) = ", len(contig_list_result))

output_file = open("output_contig_list.txt", "w")

output_file.write(str(contig_list_result).replace("\"", "").replace("\'","").replace("\"", "").replace("\'","").replace(",","").replace("[", "").replace("]", ""))
